...
public class IHostDeathInventory extends RInventory {
    ...
    public IHostDeathInventory(UHCAPI api) {
    ...
    private Consumer<InventoryClickEvent> editInventory() {
        ...
        return e -> {
            ...
            if (getApi().getHostManager().getDeathInventory() != null) {
                e.getWhoClicked().sendMessage((String) getApi().getGameMessages().getObject("api.inventoryAlreadyEdit"));
                return;
            }

            getApi().getHostManager().setDeathInventory(e.getWhoClicked().getUniqueId());
            e.getWhoClicked().closeInventory();
            e.getWhoClicked().setGameMode(GameMode.CREATIVE);
            PlayerUtils.giveInventory((ItemStack[]) getApi().getRules().getGameInfos().getObject("api.deathInventory"), (Player) e.getWhoClicked());
